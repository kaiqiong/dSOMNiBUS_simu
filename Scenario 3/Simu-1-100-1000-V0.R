...
for ( mm in sep_index[[sss]]){
  ...
  Z <- data.frame(matrix(NA, nrow= my.samp, ncol = 1)); colnames(Z) <- c("disease")
  ...
  sim.dat<-BSMethSim_bbinom(n= my.samp, posit = pos, theta.0 =beta_all[,1], beta= beta_all[,c(2,4)], phi=phi, 
                            X = my.X, Z =Z,p0 = 0.2, p1 = 0.8,random.eff = T,  sigma.ee=1)
  ...
  out <- BSMethEM(data=dat.use, n.k = n.k,epsilon = 10^(-6)/300,p0 = 0.2,
                  p1 = 0.8,maxStep = 500, method="REML", Quasi = T, RanEff = T, reml.scale = F)
  
  
  print(Sys.time()-time.0)
  
 
  save(Y, out, file =paste0("Res_",mm,"V0Res_all_Samp",my.samp, "Simu", M, ".RData") )
}
